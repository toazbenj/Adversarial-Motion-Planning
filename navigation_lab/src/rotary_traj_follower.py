...
class Follower():
    ...
    def start(self):
        ...
        self.agent_a = message_filters.Subscriber("/mocap_node/"+ str(self.args.r_a)+ "/Odom", Odometry)
        # Subscribe to self.
        self.agent_self = message_filters.Subscriber("/mocap_node/"+ str(self.args.r_f)+ "/Odom", Odometry)
        
        # Time synchronization.
        ts = message_filters.ApproximateTimeSynchronizer([self.agent_a, self.agent_self], 1, 0.05, allow_headerless=True)
        ts.registerCallback(self.control)
        
    def control(self, agent_a, agent_self):
        # Position of the robot ahead.
        pos_ahead = agent_a.pose.pose.position
        # Position of self.
